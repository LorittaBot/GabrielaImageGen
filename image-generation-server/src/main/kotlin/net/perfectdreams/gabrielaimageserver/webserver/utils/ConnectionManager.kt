...
class ConnectionManager(
    ...
    fun isTrusted(url: String): Boolean {
        ...
        return if (domain != null) {
            trustedDomains.any { it.endsWith(domain) }
        } else {
            false
        }
    }
    ...
    fun getDomainFromUrl(url: String): String? {
        val url = try { URL(url) } catch (e: Exception) { return null }
        val host = url.host
        ...
        val isSld = SLDS.any { joined.endsWith(it) }
        val howMuchWillItTake = if (isSld) 3 else 2

        return split.takeLast(howMuchWillItTake).joinToString(".")
    }
